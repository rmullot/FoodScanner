...
Simulator gotchas (Xcode 27): two `iPhone 17` simulators exist (iOS 26.5 and 27.0), so pin the destination with `id=<udid>` or `,OS=27.0`. `xcodebuild test` can silently reuse a stale UI-test bundle (symptoms: `Executed 0 tests` for a new class, failure line numbers from the old file) — rerun with a fresh `-derivedDataPath`. To reach the Scanner without onboarding: `xcrun simctl spawn <udid> defaults write com.MULLOTRomainEI.FoodScanner hasSeenOnboarding -bool true`; reset camera state with `xcrun simctl privacy <udid> reset all <bundle id>`. Layout work must be checked on iOS 27 (floating tab bar) in portrait and landscape, not only on iOS 26.5. A UI-test runner process can write files to the host (`/private/tmp/...`), so to inspect a layout add temporary `XCUIScreen.main.screenshot()` / `app.debugDescription` dumps inside an existing UI test (new test classes are not always discovered, see above), read the frames, then revert; screenshots taken this way are raw framebuffers, rotate them for landscape (`sips -r 270`). A long `xcodebuild test` can hang if the Mac sleeps: shut down and boot the simulator, then rerun.
...
- `Scanner/` — `ScannerScreenView` (AVFoundation capture via `CameraPreviewView`, `FSBarcodeField` / `FSKeypad` manual entry) + `ScannerViewModel`. Coordinator-agnostic: it takes its VM plus an `onProductFound` closure; the `NavigationStack` and destination live in `ScannerCoordinatorView`. Layout is size-class driven (`ScannerLayout`): camera as a full-bleed background layer, and inside the `GeometryReader` (which already excludes the safe areas) a status/placeholder area plus the bottom-anchored input panel (side by side only at regular width + non-compact height, i.e. iPad / iPhone Duo inner display); at compact height with the keypad open the panel goes two-column and the navigation bar is hidden. The panel's real height is measured (`PanelHeightKey`) and used as bottom padding of the status area, whose placeholder degrades through `ViewThatFits` variants; `ScannerLayout.panelMaxHeight` caps the panel and must leave room for the barcode validation message (~45pt), otherwise the panel falls back to a scroll view and XCUITest taps fail. Don't split the height between the status area and the panel with a plain `VStack`, and don't use `safeAreaInset`/overlay on a full-bleed view: both pushed the panel under the iOS 27 floating tab bar. Never use `UIScreen.main.bounds` or `UIDevice.orientation` for layout; camera rotation goes through `AVCaptureDevice.RotationCoordinator`.
...
- `apple-docs-referent` — referent for Apple's documentation (HIG + developer docs, e.g. iPhone Duo, split views, toolbars). Reads pages only as Markdown via the URL rewrite (insert `tutorials/data/` before `design/` or `documentation/`, append `.md`), never HTML or `WebFetch`; cites section + URL; never denies a product/API without trying its `.md` URL. Read-only. The other agents consult it before relying on an Apple guideline or API.
- Apple-authored skills in `.claude/skills/` (exported with `xcrun agent skills export --output-dir <absolute path>`): `swiftui-specialist`, `swiftui-whats-new-27`, `uikit-app-modernization`, `device-interaction`. They supersede model memory on their topics.
