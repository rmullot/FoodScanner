...
1. What data flows through: scanned barcode (`ScannerViewModel`), product data (`FoodStruct`/`Food`), camera access (`AVFoundation`, `NSCameraUsageDescription` in `Info.plist`).
2. Where it's stored: `CacheManager` (actor) — local Realm, "per-user Realm file" (check in the current code whether a user identifier is actually generated/stored, and in what form).
...
4. **Local storage (Realm) and security** — retention period of data in Realm (no purge = unlimited retention to question); the default Realm file is encrypted at rest (AES-256 via `Realm.Configuration.encryptionKey`, key in the Keychain, generated on first launch — see CLAUDE.md's SECURITY note and `RealmEncryptionKeyStore`), so verify this is actually wired on the production path (not silently bypassed) rather than assuming it from the doc alone; absence of unjustified sensitive data in cleartext.
