...
7. **Repo conventions + SwiftLint** — conformance to CLAUDE.md (no singletons/`.sharedInstance`: services are protocol-typed and injected via `InjectionManager`; `async`/`await` + actors, no GCD/completion handlers; each screen's `<Screen>ViewModel` is a plain `ObservableObject` with `@Published` state; Realm models produced only via `CacheManager`) and to the repo's `.swiftlint.yml`. Re-read CLAUDE.md at audit time rather than trusting this list — it summarizes conventions that can evolve. Run `swiftlint lint` (or `swiftlint lint --path <file>` for a reduced scope) via Bash if the tool is available, and cite the actual violations the tool returns, not assumptions.
