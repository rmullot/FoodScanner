...
final class ScannerLayoutTests: XCTestCase {
    ...
    func test_isSideBySide_whenCompactHeight_isTrue() {
        XCTAssertTrue(ScannerLayout(horizontalSizeClass: .compact, verticalSizeClass: .compact).isSideBySide)
    }
    ...
    func test_panelMaxHeight_stackedIsThreeQuartersOfContainer() {
        let layout = ScannerLayout(horizontalSizeClass: .compact, verticalSizeClass: .regular)

        XCTAssertEqual(layout.panelMaxHeight(containerHeight: 800), 600)
    }
